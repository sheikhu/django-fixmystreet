# -*- coding: utf-8 -*-
import datetime
from django_fixmystreet.fixmystreet.models import ReportAttachment
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        reports_refused = orm['fixmystreet.Report'].objects.filter(refusal_comment__isnull=False)

        for report in reports_refused:

            report.refusal_comment.type = ReportAttachment.REFUSED
            report.refusal_comment.save()

        reports_mark_as_done = orm['fixmystreet.Report'].objects.filter(mark_as_done_comment__isnull=False)

        for report in reports_mark_as_done:
            report.mark_as_done_comment.type = ReportAttachment.MARK_AS_DONE
            report.mark_as_done_comment.save()
    ...
    def backwards(self, orm):
        ...
        for report in reports_refused:
            report.refusal_comment.type = ReportAttachment.DOCUMENTATION
            report.refusal_comment.save()

        reports_mark_as_done = orm['fixmystreet.Report'].objects.filter(mark_as_done_comment__isnull=False)

        for report in reports_mark_as_done:
            report.mark_as_done_comment.type = ReportAttachment.DOCUMENTATION
            report.mark_as_done_comment.save()
