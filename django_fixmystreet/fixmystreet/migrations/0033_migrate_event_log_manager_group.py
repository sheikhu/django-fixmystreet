...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        """
        initialize group that is resposible instead of manager suer

        SELECT event_type, user_id, related_new_id, related_content_type_id, event_at
        FROM fixmystreet_reporteventlog log
        WHERE event_type IN (4, 7) and related_new_id IS NULL
        ORDER BY event_at
        """
        events = orm['fixmystreet.ReportEventLog'].objects.filter(related_new_id=None, event_type__in=(4, 7))
        for e in events:
            if e.user and e.user.fmsuser.memberships.all():
                e.related_new_id = e.user.fmsuser.memberships.all()[0].organisation.id
                e.related_content_type_id = 15
                e.save()
            else:
                print "event skipped", e.id, e.report.id
