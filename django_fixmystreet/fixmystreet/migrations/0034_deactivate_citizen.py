...
class Migration(SchemaMigration):
    ...
    JOIN auth_user ON id=user_ptr_id
    WHERE
        agent=False AND
        manager=False AND
        leader=False AND
        ...
        organisation_id IS NULL AND
        is_superuser=False;
    """
    def forwards(self, orm):
        citizens = orm['fixmystreet.FMSUser'].objects.filter(
            agent=False,
            manager=False,
            leader=False,
            applicant=False,
        ...
        print 'deactivate citizens', citizens.count()
        citizens.update(is_active=False, password="!")
