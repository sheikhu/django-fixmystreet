...
from django_fixmystreet.fixmystreet.models import OrganisationEntity, Report, ReportEventLog
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        sibelga_entity.type      = OrganisationEntity.REGION
        sibelga_entity.region    = True
        sibelga_entity.applicant = False
        sibelga_entity.save()

        # 2/ Create Sibelga Service group
        sibelga_service            = orm['fixmystreet.OrganisationEntity']()
        sibelga_service.dependency = sibelga_entity
        sibelga_service.type       = OrganisationEntity.DEPARTMENT
        sibelga_service.commune    = True

        sibelga_service.phone      = sibelga_applicant.phone
        sibelga_service.email      = sibelga_applicant.email

        sibelga_service.name_fr    = "Service %s" % sibelga_applicant.name_fr
        sibelga_service.name_nl    = "Service %s" % sibelga_applicant.name_nl
        sibelga_service.slug_fr    = "service-%s" % sibelga_applicant.slug_fr
        sibelga_service.slug_nl    = "service-%s" % sibelga_applicant.slug_nl

        sibelga_service.save()

        # 3/ Add membership sibelga user to group
        for idx, user in enumerate(orm['fixmystreet.FMSUser'].objects.filter(username__endswith="@sibelga.be")):
            user.organisation = sibelga_entity

            sibelga_organisation_membership              = orm['fixmystreet.UserOrganisationMembership']()
            sibelga_organisation_membership.user         = user
            sibelga_organisation_membership.organisation = sibelga_service

            if (idx == 0):
                sibelga_organisation_membership.contact_user = True
                user.leader = True

            user.save()
            sibelga_organisation_membership.save()


        # 4/ Migrate all reports assigned to Sibelga by transferering to Sibelga
        #~ reports = orm['fixmystreet.Report'].objects.filter(contractor=sibelga_applicant, status__in=Report.REPORT_STATUS_IN_PROGRESS, id=10838)
        reports = orm['fixmystreet.Report'].objects.filter(contractor=sibelga_applicant, status__in=Report.REPORT_STATUS_IN_PROGRESS)

        for report in reports:
            #~ print report.id, report.contractor.id, report.responsible_department.id, report.responsible_entity.id, report.status

            report.contractor             = None
            report.responsible_department = sibelga_service
            report.responsible_entity     = sibelga_entity
            report.status                 = Report.MANAGER_ASSIGNED
            report.save()

            # 5/ Add log in the history
            #~ print '   add log in history'

            latest_event = orm['fixmystreet.ReportEventLog'].objects.filter(report=report).latest('event_at')

            report_event_log                = orm['fixmystreet.ReportEventLog']()

            report_event_log.report         = report
            report_event_log.event_type     = ReportEventLog.ENTITY_ASSIGNED
            report_event_log.organisation   = report.responsible_entity
            report_event_log.user           = latest_event.user

            report_event_log.status_old     = latest_event.status_new
            report_event_log.status_new     = ReportEventLog.MANAGER_ASSIGNED

            report_event_log.related_old_id = latest_event.related_new_id
            report_event_log.related_new_id = sibelga_service.id
            report_event_log.related_content_type = orm['contenttypes.ContentType'].objects.get(id=15)

            report_event_log.save()

            # Set the same date +1 sec than latest event
            orm['fixmystreet.ReportEventLog'].objects.filter(pk=report_event_log.pk).update(event_at=latest_event.event_at + datetime.timedelta(seconds=1))

    def backwards(self, orm):
        pass

        # Backwards only for test

        # Retrieve entities
        sibelga_entity  = orm['fixmystreet.OrganisationEntity'].objects.get(name_fr='Sibelga Eclairage', type=OrganisationEntity.REGION)
        sibelga_service = orm['fixmystreet.OrganisationEntity'].objects.get(dependency=sibelga_entity, type=OrganisationEntity.DEPARTMENT)
        sibelga_organisation_membership_list = orm['fixmystreet.UserOrganisationMembership'].objects.filter(organisation=sibelga_service)

        # Restore applicant
        sibelga_applicant           = sibelga_entity
        sibelga_applicant.type      = OrganisationEntity.APPLICANT
        sibelga_applicant.region    = False
        sibelga_applicant.applicant = True
        sibelga_applicant.save()
#~
        #~ # Restore all reports assigned to Sibelga by transferering to Sibelga
        #~ reports = orm['fixmystreet.Report'].objects.filter(responsible_entity=sibelga_entity, responsible_department=sibelga_service, status__in=Report.REPORT_STATUS_IN_PROGRESS, id=10838)
#~
        #~ for report in reports:
            #~ print report.id, report.contractor, report.responsible_department.id, report.responsible_entity.id, report.status
#~
            #~ report.contractor             = sibelga_applicant
            #~ report.responsible_department = orm['fixmystreet.OrganisationEntity'].objects.get(id=81)
            #~ report.responsible_entity     = orm['fixmystreet.OrganisationEntity'].objects.get(id=18)
            #~ report.status                 = Report.APPLICANT_RESPONSIBLE
            #~ report.save()
#~
            #~ # Fix log in the history
            #~ latest_report_event_log = orm['fixmystreet.ReportEventLog'].objects.filter(report=report, event_type=ReportEventLog.ENTITY_ASSIGNED, related_new_id=sibelga_service.id).latest('event_at')
#~
            #~ latest_report_event_log.delete()
#~
        # Delete membership and the group
        sibelga_organisation_membership_list.delete()
        sibelga_service.delete()
