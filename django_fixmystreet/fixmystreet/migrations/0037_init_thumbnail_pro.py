...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for report in orm['fixmystreet.Report'].objects.all():
            ...
            images_pro = report.attachments.filter(logical_deleted=False, reportcomment=None, reportfile__isnull=False)

            if images_pro.exists():

                try:
                    splitted_filename = list(os.path.splitext(images_pro[0].reportfile.image.url))
                    splitted_filename.insert(1, '.thumbnail')
                    thumbnail_pro = ''.join(splitted_filename)

                    report.thumbnail_pro = thumbnail_pro
                    report.save()
