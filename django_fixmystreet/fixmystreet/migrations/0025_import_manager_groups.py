...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        for m in managers:
            if m.categories.count() > 0 or m.reports_in_charge.count() > 0:
                ...
                print u"--- creating membership"
                group.memberships.create(user=m, contact_user=True)
                print u"--- linking dispaching categories"
                group.dispatch_categories.add(*m.categories.all())
                print group.dispatch_categories.count()
                m.categories.clear()  # for dev
                print u"--- linking reports"
                group.reports_in_department.add(*m.reports_in_charge.all())
                print group.reports_in_department.count()
                m.reports_in_charge.clear()  # for dev
        ...
        for c in contractors:
            ...
            for i, member in enumerate(c.workers.all()):
                contact = (i == 0)  # considere the first as contact
                c.memberships.create(user=member, contact_user=contact)
                if contact:
    ...
    models = {
        ...
        u'fixmystreet.historicalreport': {
            ...
            'responsible_department_id': ('django.db.models.fields.IntegerField', [], {'db_index': 'True', 'null': 'True', 'blank': 'True'}),
        ...
        u'fixmystreet.organisationentity': {
            ...
            'dispatch_categories': ('django.db.models.fields.related.ManyToManyField', [], {'symmetrical': 'False', 'related_name': "'assinged_to_department'", 'blank': 'True', 'to': u"orm['fixmystreet.ReportCategory']"}),
            'email': ('django.db.models.fields.EmailField', [], {'max_length': '75', 'null': 'True', 'blank': 'True'}),
            'feature_id': ('django.db.models.fields.CharField', [], {'max_length': '25', 'null': 'True', 'blank': 'True'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'modified': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now', 'blank': 'True'}),
