...
def show( request, ward_id ):
    ...
    reports = list(Report.objects.filter(ward = ward).extra( select = { 'status' : """
        CASE 
        WHEN age( clock_timestamp(), created ) < interval '1 month' AND is_fixed = false THEN 'New Problems'
        WHEN age( clock_timestamp(), created ) >= interval '1 month' AND is_fixed = false THEN 'Older Unresolved Problems'
        ...
        'status_int' : """
        CASE 
        WHEN age( clock_timestamp(), created ) < interval '1 month' AND is_fixed = false THEN 0
        WHEN age( clock_timestamp(), created ) >= interval '1 month' AND is_fixed = false THEN 1
