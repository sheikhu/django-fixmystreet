...
function getImageData(oImg, fncCallback, index)
{
    BinaryAjax(
        oImg.src,
        function(oHTTP) {
            ...
            oImg.exifdata = oEXIF || {};
            if (fncCallback) fncCallback(oEXIF, index);
        }
    );
}
...
EXIF.getData = function(oImg, fncCallback, index)
{
    if (!oImg.complete) return false;
    if (!imageHasData(oImg)) {
        getImageData(oImg, fncCallback, index);
    } else {
...
$.fn.exifLoad = function(fncCallback) {
    return this.each(function(index ) {
        EXIF.getData(this, fncCallback, index);
    });
};

$.fn.getMyExifData = function(){
    var image = this.get(0);
    EXIF.getData(image, function(){},0);
    var exifdata = findEXIFinJPEG(this);
    alert("we zijn klaar");
    alert(exifdata);
}
